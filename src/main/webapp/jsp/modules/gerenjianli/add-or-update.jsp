...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑个人简历</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>个人简历管理</span></li>
								<li class="breadcrumb-item active"><span>编辑个人简历</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">个人简历信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>教育经历</label>
										<input id="jiaoyujingli" name="jiaoyujingli" class="form-control" placeholder="教育经历">
									</div>
									<div class="form-group col-md-6">
										<label>工作经历</label>
										<textarea name="gongzuojingli" class="form-control" id="gongzuojingli" rows="3"></textarea>
									</div>
									<div class="form-group col-md-6">
										<label>技能</label>
										<input id="jineng" name="jineng" class="form-control" placeholder="技能">
									</div>
									<div class="form-group col-md-6">
										<label>自我评价</label>
										<input id="ziwopingjia" name="ziwopingjia" class="form-control" placeholder="自我评价">
									</div>
										
											<div class="form-group  col-md-12">
												<label>工作业绩</label>
												<div><div class="upload"><input id="gongzuoyejiupload" name="file" type="file">选择文件</div>
												<script id="gongzuoyejiEditor" type="text/plain" style="width:800px;height:230px;"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var gongzuoyejiUe = UE.getEditor('gongzuoyejiEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="gongzuoyeji" id="gongzuoyeji-input">
												</div>
											</div>	
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "gerenjianli/info/" + id,
		...
		function upload(){
			...
			$('#gongzuoyejiupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('gongzuoyejiEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		...
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("gerenjianli/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.gongzuoyeji != null && ruleForm.gongzuoyeji != 'null' && ruleForm.gongzuoyeji != ''){
				var gongzuoyejiMes = '' + ruleForm.gongzuoyeji;
				var gongzuoyejiUeditor = UE.getEditor('gongzuoyejiEditor');
				gongzuoyejiUeditor.ready(function() {
					gongzuoyejiUeditor.setContent(gongzuoyejiMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('gongzuoyejiEditor').hasContents()){
				$('#gongzuoyeji-input').attr('value',UE.getEditor('gongzuoyejiEditor').getPlainTxt());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("gerenjianli/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dataBind(){
			...
			$("#jiaoyujingli").val(ruleForm.jiaoyujingli);	
			$("#gongzuojingli").val(ruleForm.gongzuojingli);
			$("#gongzuoyeji").val(ruleForm.gongzuoyeji);	
			$("#jineng").val(ruleForm.jineng);	
			$("#ziwopingjia").val(ruleForm.ziwopingjia);	
		}		
		//图片显示
		function showImg(){
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.jiaoyujingli != null && res.data.jiaoyujingli != ''){

					$("#jiaoyujingli").val(res.data.jiaoyujingli);
					}
					if(res.data != null && res.data != '' && res.data.gongzuojingli != null && res.data.gongzuojingli != ''){

					$("#gongzuojingli").val(res.data.gongzuojingli);
					}
					if(res.data != null && res.data != '' && res.data.gongzuoyeji != null && res.data.gongzuoyeji != ''){

					$("#gongzuoyeji").val(res.data.gongzuoyeji);
					}
					if(res.data != null && res.data != '' && res.data.jineng != null && res.data.jineng != ''){

					$("#jineng").val(res.data.jineng);
					}
					if(res.data != null && res.data != '' && res.data.ziwopingjia != null && res.data.ziwopingjia != ''){

					$("#ziwopingjia").val(res.data.ziwopingjia);
					}
				}
			});  
            }

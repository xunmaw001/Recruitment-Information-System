...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">个人简历管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>个人简历列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">个人简历列表</h3>							
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('jiaoyujingli')">教育经历<i id="jiaoyujingliIcon" class="fa fa-sort"></i></th>

											<th>工作经历</th>
											<th onclick="sort('gongzuoyeji')">工作业绩<i id="gongzuoyejiIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('jineng')">技能<i id="jinengIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('ziwopingjia')">自我评价<i id="ziwopingjiaIcon" class="fa fa-sort"></i></th>
											<th>操作</th>
										</tr>
									</thead>
									<tbody>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var jiaoyujingliCell  = document.createElement('td');

						 jiaoyujingliCell.innerHTML = item.jiaoyujingli;


			row.appendChild(jiaoyujingliCell);
			var gongzuojingliCell  = document.createElement('td');

						  if(item.gongzuojingli != null && item.gongzuojingli != "" && item.gongzuojingli.length >= 11){
			  	gongzuojingliCell.innerHTML = item.gongzuojingli.substring(0, 10) + "..."; 
			  }else{
			  	gongzuojingliCell.innerHTML = item.gongzuojingli;
			  }


			row.appendChild(gongzuojingliCell);
			var gongzuoyejiCell  = document.createElement('td');

						  //如果图文详情包含图片
			  if(item.gongzuoyeji && item.gongzuoyeji.indexOf('img') != -1){
				//暂时只考虑图片+文字 和 文字+图片的情况
				var beginIndex = item.gongzuoyeji.indexOf('<img');
				var endIndex = item.gongzuoyeji.indexOf('>');				
				gongzuoyejiCell.innerHTML = item.gongzuoyeji.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
			  }else{
			  	 if(item.gongzuoyeji != null && item.gongzuoyeji != "" && item.gongzuoyeji.length >= 11){
			  		gongzuoyejiCell.innerHTML = item.gongzuoyeji.substring(0, 10) + "..."; 
			  	}else{
			  		gongzuoyejiCell.innerHTML = item.gongzuoyeji;
			 	}
			  }
				  


			row.appendChild(gongzuoyejiCell);
			var jinengCell  = document.createElement('td');

						 jinengCell.innerHTML = item.jineng;


			row.appendChild(jinengCell);
			var ziwopingjiaCell  = document.createElement('td');

						 ziwopingjiaCell.innerHTML = item.ziwopingjia;


			row.appendChild(ziwopingjiaCell);
            ...
            crossBtn1.innerHTML = "邀请"
			crossBtn1.setAttribute("type","button");
			crossBtn1.setAttribute("class","btn btn-primary btn-sm");
			crossBtn1.setAttribute("onclick","crossTable(" + item.id + ",\"yingpinyaoqing\")");
			if(crossBtnControl('邀请') == true){
	...
	// 导出
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("gerenjianli/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"gerenjianli");
